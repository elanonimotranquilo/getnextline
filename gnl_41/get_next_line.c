...
/*   Updated: 2022/11/24 18:33:54 by novasol          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if ((fd < 0) || (BUFFER_SIZE <= 0))
		return (NULL);
	read_aux = ft_save_line(sline);
	if (read_aux)
	{
		out_line = ft_get_line(sline);
		ft_free_clean(&sline);
		sline = ft_strjoin(read_aux, NULL);
		return (ft_free_clean(&read_aux), out_line);
	}
	ft_free_clean(&read_aux);
	n_read = 1;
	read_aux = ft_read_fd(fd, &n_read, &sline);
	if (n_read == -1)
		return (ft_free_clean(&sline), NULL);
